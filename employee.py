...
class Employee:
    ...
    def __str__(self):
        if(self.contract_type == "Monthly"):
            ...
            if(self.commission_bonus > 0):
                return (f'{self.name} works on a contract of {self.hours} at {self.hourly_pay}/hour and recieves a commission bonus {self.commission_bonus}. Their total pay is {self.get_pay()}.')
            elif(self.commission_contracts > 0):
                return (f'{self.name} works on a contract of {self.hours} at {self.hourly_pay}/hour and recieves a commission for {self.commission_contracts} contract(s) at {self.commission_hourly_pay}/contract. Their total pay is {self.get_pay()}.')
            else:
